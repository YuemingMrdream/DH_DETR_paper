...
def relation_net_assignment(c, alive_mask, cur_all_mask, valid_mask):
    c = c.numpy()
    i_0, j_0 = masked_assignment(c[alive_mask], valid_mask[alive_mask])  # alive_mask 是高置信度的
...
def cascade_net_assignment(c, alive_mask, cur_all_mask, valid_mask, cascade_mask):
    c = c.numpy()
    i_0, j_0 = masked_assignment(c[alive_mask], valid_mask[alive_mask])  # alive_mask 是高置信度的
    selected = np.zeros(c.shape[1], dtype=bool)
    selected[j_0] = 1
    ...
    i_1=np.array([])
    j_1=np.array([])
    for k in range(len(cascade_mask)):
        cas_mask = cascade_mask[k].cpu().numpy()
        i_, j_ = masked_assignment(c[cas_mask][:, ~selected], valid_mask[cas_mask][:, ~selected])
        i_ = np.where(cas_mask)[0][i_]
        j_ = np.where(~selected)[0][j_]
        selected[j_]=1
        i_1 = np.concatenate((i_1, i_))
        j_1 = np.concatenate((j_1, j_))
    return i_1, j_1
...
class HungarianMatcher(nn.Module):
    ...
    def forward(self, outputs, targets):
        with torch.no_grad():
            ...
            # Final cost matrix
            C = self.cost_bbox * cost_bbox + self.cost_class * cost_class + self.cost_giou * -cost_giou   # adding weight
            C = C.view(bs, num_queries, -1).cpu()       # loss
            ...
            if outputs['mask'] is not None:
                ...
                alive_mask = outputs['mask']['seed_mask'].cpu().numpy()   # seed_mask 是置信度大于某一值的 config.score_thr
                cur_all_mask = outputs['mask']['mask'].cpu().numpy()      # (~seed_mask) & (scores > config.floor_thr)
                # +++++
                # cascade_mask = outputs['mask']['cascade_mask']
                indices = [relation_net_assignment(c[i], alive_mask[i], cur_all_mask[i], valid_mask[i]) for i, c in enumerate(C.split(sizes, -1))]
                # indices = [cascade_net_assignment(c[i], alive_mask[i], cur_all_mask, valid_mask[i], cascade_mask[i]) for i, c in enumerate(C.split(sizes, -1))]
            else:
